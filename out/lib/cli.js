...
e.on('man', (_        ) => cli.responders.help());
e.on('help', (_        ) => cli.responders.help());
e.on('exit', (userInput        ) => cli.responders.exit());
e.on('stats', (userInput        ) => cli.responders.stats());
e.on('menu', (userInput        ) => cli.responders.menu());
e.on('orders', (userInput        ) => cli.responders.orders(userInput));
e.on('users', (userInput        ) => cli.responders.users(userInput));
...
const printObjectAsLines = (title        , obj                             ) => {
...
cli.verticalSpace = (lines         = 1) => new Array(lines).forEach(i => console.log(i));
...
cli.responders.users = async (userInput        ) => {
...
cli.responders.orders = async (userInput        ) => {
    const splitInput = userInput.split(' ');
...
cli.processInput = (trimmedUserInput        ) => {
