...
lib.create = (dir: string, file: string, data: Object, callback: (_: string | boolean) => void) => {
...
lib.read = (dir: string, file: string, callback: (err: string, data: string) => void) => {
    fs.readFile(lib.baseDir + dir + '/' + file + '.json', 'utf8', (err, data) => callback(JSON.stringify(err), data));
};

lib.update = (dir: string, file: string, data: Object, callback: (err: string | false) => void) => {
...
lib.delete = (dir: string, file: string, callback: (err: string | false) => void) => {
