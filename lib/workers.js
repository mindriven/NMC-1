...
import type {Order, User} from './server/handlers';

const workers = {};
workers.sendInvoices = async function () {
    const allOrdersIds = (await _data.listFiles('orders')).map(name => name.replace('.json', ''));
    const allOrders: Order[] = (await Promise.all(allOrdersIds.map(async id => ({order: await _data.read('orders', id), id}))))
...
function createMailBody(order: Order, user: User): string {
...
function getMailgunPostOptions(requestData: string): Object {
    const options = {
